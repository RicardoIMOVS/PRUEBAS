...
export default class Earth {
  ...
  constructor(options: Options) {
    ...
    this.uniforms = {
      glowColor: { value: new Color(0x0ea5e9) },
      // glowColor: { value: new Color(0x1e3a8a) },
  ...
  createEarth() {
    ...
    const pointMaterial = new PointsMaterial({
      color: 0x7497d2, // gris azulado tenue
      transparent: true,
      opacity: 0.02,
      size: 0.01,
    });
  ...
  createEarthAperture() {
    const material = new ShaderMaterial({
      uniforms: {
        glowColor: { value: new Color(0x1d4ed8) },
  ...
  render() {
    ...
    this.waveMeshArr.forEach((wave: any) => {
      wave.userData.scale += 0.01;

      if (wave.userData.scale > 2) {
        wave.userData.scale = 1;
      }

      const s = wave.userData.size * wave.userData.scale;
      wave.scale.set(s, s, s);

      wave.material.opacity = 1.5 - wave.userData.scale;
    });
  }
}